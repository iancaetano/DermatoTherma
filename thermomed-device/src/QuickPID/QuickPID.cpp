...
QuickPID::QuickPID(float* Input, float* Output, float* Setpoint,
                   float Kp, float Ki, float Kd, float POn = 1.0, float DOn = 1.0,
...
QuickPID::QuickPID(float* Input, float* Output, float* Setpoint,
  ...
  : QuickPID::QuickPID(Input, Output, Setpoint, Kp, Ki, Kd, pOn = 1.0, dOn = 1.0, ControllerDirection = DIRECT) {
}
...
bool QuickPID::Compute() {
  ...
  if (mode == TIMER || timeChange >= sampleTimeUs) {
    ...
    float outMax_i = (input-30.)*0.3 + 3.;
    if (outMax_i < 0.) outMax_i = 0.;
    outputSum = constrain(outputSum, outMin, outMax_i);                             // integral anti-windup clamp
...
******************************************************************************/
void QuickPID::SetTunings(float Kp, float Ki, float Kd, float POn = 1.0, float DOn = 1.0) {
